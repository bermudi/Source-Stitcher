...
class ProjectFileWalker:
    ...
    def __init__(self, config: WorkerConfig, progress_callback: Optional[callable] = None):
        """
            ...
            try:
                ...
                if is_regular_file:
                    ...
                    if not self._is_directory_ignored(path):
                        ...
                        logger.debug(f"Added {len(dir_files)} files from directory: {path}")
                
            except (OSError, ValueError) as e:
                logger.error(f"Error discovering files in {path}: {e}")
                continue
            except Exception as e:
                logger.error(f"Unexpected error discovering files in {path}: {e}", exc_info=True)
                continue
        
        end_time = time.time()
        total_count = len(discovered_files)
        logger.info(f"File discovery completed: {total_count} files found in {end_time - start_time:.2f}s")
    ...
    def _discover_directory_recursive(
        ...
        def walk_error_handler(error: OSError) -> None:
            logger.warning(f"Permission/OS error during discovery walk below {dir_path}: {error}")
        ...
        for root, dirs, files in os.walk(
            ...
            try:
                ...
                logger.warning(f"Could not make path relative during discovery: {root_path}. Skipping subtree.")
                dirs[:] = []
                continue
            
            # Filter directories in-place
            self._filter_directories(dirs, root_relative_to_base, root_relative_to_current, current_dir_ignore_spec)
            ...
            for file_name in files:
                ...
                try:
                    ...
                    if self._should_include_file(full_path, st, seen, current_dir_ignore_spec, root_relative_to_current):
                        discovered_files.append(full_path)
                        seen.add((st.st_dev, st.st_ino))
                        logger.debug(f"Discovered file: {full_path}")
                except (OSError, ValueError) as e:
                    logger.warning(f"Could not process file during discovery: {full_path}, error: {e}")
                    continue
    ...
    def _filter_directories(
        ...
        for d in original_dirs:
            if self._is_directory_ignored_by_name(d, root_relative_to_base, root_relative_to_current, current_dir_ignore_spec):
    ...
    def _is_directory_ignored(self, dir_path: Path) -> bool:
        ...
        """
        try:
            rel_path_str = str(dir_path.relative_to(self.config.generation_options.base_directory)) + "/"
        except ValueError:
            return False
        
        # Check project ignore patterns
        if (self.config.filter_settings.ignore_spec and 
            self.config.filter_settings.ignore_spec.match_file(rel_path_str)):
            return True
        
        # Check global ignore patterns
        if (self.config.filter_settings.global_ignore_spec and 
            self.config.filter_settings.global_ignore_spec.match_file(rel_path_str)):
            return True
    ...
    def _is_directory_ignored_by_name(
        ...
        # Check project ignore patterns
        if (self.config.filter_settings.ignore_spec and 
            self.config.filter_settings.ignore_spec.match_file(full_dir_path_str)):
            return True
        
        # Check local ignore patterns
        if (current_dir_ignore_spec and 
            current_dir_ignore_spec.match_file(str(root_relative_to_current / dir_name) + "/")):
            return True
        
        # Check global ignore patterns
        if (self.config.filter_settings.global_ignore_spec and 
            self.config.filter_settings.global_ignore_spec.match_file(full_dir_path_str)):
            return True
    ...
    def _should_include_file(
        ...
        try:
            relative_path_to_base = file_path.relative_to(self.config.generation_options.base_directory)
        except ValueError:
        ...
        # Check project ignore patterns
        if (self.config.filter_settings.ignore_spec and 
            self.config.filter_settings.ignore_spec.match_file(relative_path_str)):
            return False
        ...
        if current_dir_ignore_spec and root_relative_to_current is not None:
            try:
                relative_path_to_current = file_path.relative_to(
                    self.config.generation_options.base_directory / root_relative_to_current
                )
        ...
        # Check global ignore patterns
        if (self.config.filter_settings.global_ignore_spec and 
            self.config.filter_settings.global_ignore_spec.match_file(relative_path_str)):
            return False
