...
class LanguageDefinitionLoader:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        logger.debug(f"LanguageDefinitionLoader initialized with config path: {self.config_path}")
    ...
    def load_definitions(self) -> Dict[str, List[str]]:
        ...
        if self._definitions is None:
            self._definitions = self._load_from_toml() or self._get_fallback_definitions()
    ...
    def _load_from_toml(self) -> Optional[Dict[str, Dict[str, Union[List[str], str]]]]:
        ...
        try:
            ...
            logger.info(f"Successfully loaded language definitions from {self.config_path}")
            return data
        ...
        for items in definitions.values():
            for item in items:
                if not item.startswith(".") and item != "*other*":
                    filenames.add(item.lower())  # Store in lowercase for case-insensitive matching
        ...
        for lang_name, items in definitions.items():
            for item in items:
                if item.startswith(".") and item.lower() == file_ext:
                    return lang_name
                elif not item.startswith(".") and item != "*other*" and item.lower() == file_name:
                    return lang_name
    ...
    def create_default_toml_file(self, output_path: Optional[Path] = None) -> Path:
        ...
        for lang_name, lang_data in definitions.items():
            ...
            section_name = lang_name.replace("/", "_").replace(" ", "_").replace("-", "_")
            ...
            if lang_data.get("filenames"):
                filename_list = ", ".join(f'"{name}"' for name in lang_data["filenames"])
