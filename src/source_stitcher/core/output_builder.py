...
class HeaderBuilder:
    ...
    def __init__(
        self, base_dir: Path, file_list: List[Path], selected_langs: List[str]
    ):
    ...
    def build(self) -> str:
        ...
        if not self.selected_langs:
            ...
            header_lines.append(
                f"# Selected file types: {', '.join(self.selected_langs)}"
            )
...
class ContentStreamer:
    ...
    def stream_files(
        ...
        logger.info(
            f"Content streaming completed: {processed_count}/{total} files processed"
        )
